...
if (isset($_FILES['fileToUpload'])) {
    ...
    $category_id = $ID['id'];
    $uploadOkay = true;
    echo '<div class="alert right-side body">';

    $allowed = array('pdf');
    $ext = strtolower(pathinfo($_FILES['fileToUpload']['name'], PATHINFO_EXTENSION));
//Check file type
    if (!in_array($ext, $allowed)) {
        echo(xlt('Fout - Onjuist bestandstype - Upload een PDF-bestand'));
        //echo(xlt('Error - Wrong File type - Please Upload a PDF file'));
        echo '</div>';
        $uploadOkay = false;
    }
//Checks for Executables
    if (preg_match("/(.*)\.(php|php3|php4|php5|php7)$/i", $_FILES['fileToUpload']['name']) !== 0) {
        echo(xlt('Onjuist bestandstype'));
        //echo(xlt('Executables not allowed'));
        echo '</div>';
        $uploadOkay = false;
    }
//Checks size
    if ($_FILES["fileToUpload"]["size"] > 64000000) {
        echo(xlt('Sorry, je bestand is te groot'));
        //echo(xlt('Sorry, your file is too large.'));
        echo '</div>';
        $uploadOkay = false;
    }

    if (!empty($_FILES) && $uploadOkay) {
    ...
    <aside class="right-side mt-3">
        ...
        <section class="container" id="cardgroup">
            ...
            <div class="collapse" id="lists">
                ...
                <div class="card">
                    <header class="card-header bg-primary text-light"><?php echo xlt('Upload mislukt'); ?>  </header>
                    <div id="upload_failed" class="card-body bg-light"></div>
                </div>
                <div class="card">
                    <header class="card-header bg-primary text-light"><?php echo xlt('Upload oké'); ?>  </header>
                    <div id="upload_okay" class="card-body bg-light"></div>
                </div>
            </div><!-- /.lists -->
            <?php if ($GLOBALS['allow_portal_appointments']) { ?>
            <div class="collapse mt-2" id="appointmentcard">
                ...
                <?php if (!empty($GLOBALS['portal_onsite_document_download'])) {
                    ...
                    <div class="card collapse" id="downloadcard">
                        <header class="card-header bg-primary text-light"> <?php echo xlt('Download Documenten'); ?> </header>
                ...
                <?php } ?>
                <?php if (!empty($GLOBALS['portal_onsite_document_upload'])) {
                    ?>
                    <div class="card collapse" id="uploadcard">
                        <header class="card-header bg-primary text-light"> <?php echo xlt('Uploaden Documenten'); ?> </header>
                        <div id="docsupload" class="card-body">
                            <div>
                                <span class="text"><?php echo xlt('Uploaden <b> alleen</b> PDF documenten'); ?></span>
