...
<head>
    ...
    function emailLogin($patient_id, $datum, $tijd, $provider, $catigory)
    {
        $patientData = sqlQuery("SELECT * FROM `patient_data` WHERE `pid`=?", array($patient_id));
        $facility = sqlQuery("Select * From facility");
        $provider_name = sqlQuery("SELECT fname, mname, lname FROM users WHERE  authorized != 0 AND active = 1 AND id =?", array($provider));
        $cat = sqlQuery("SELECT * FROM openemr_postcalendar_categories where pc_catid =?", array($catigory));
        $nl_date = date('d-m-Y', strtotime($datum));
        ...
        if (!empty($patientData['mname'])) {
            $pt_name = $patientData['fname'] . ' ' . $patientData['mname'] . ' ' . $patientData['lname'];
        } else {
            $pt_name = $patientData['fname'] . ' ' . $patientData['lname'];
        }

        $message = xlt("Beste ") . $patientData['title'] . ' ' . $pt_name . ":<br />";
        $message .= "<br />";
        $message .= xlt("We hebben een afspraak voor u gepland op") . "<br />";
        $message .= xlt("Datum: ") . text($nl_date) . "<br />";
        $message .= xlt("Tijd: ") . "</strong> " . text($tijd) . "<br />";
        $message .= xlt("Afspraak: ") . text($cat["pc_catname"]) . "<br />";
        $message .= xlt("Met: ") . text($provider_name['fname'] . " " . $provider_name['mname'] . " " . $provider_name['lname']) . "<br /><br />";

        if ($cat['email_filename'] != null || $cat['email_filename'] != '') {
            $message .= text("Bijgevoegd bij deze e-mail is ook wat extra informatie over uw afspraak. Neem even de tijd om het door te lezen") . "<br /><br />";
        }
        $message .= "<strong>" . xlt("Heeft u vragen of wilt u deze afspraak wijzigen, neem dan contact op") . $pref_facility['phone'] . "</strong><br />";

        $message .= text($facility['name']) . "<br />";
        $message .= text($facility['street']) . "<br />";
        $message .= text($facility['postal_code']) . " " . text($facility['city']) . "<br />";
        $phone = preg_replace('/([0-9]{3})([0-9]{2})([0-9]{2})([0-9]{3})/', '$1 - $2 $3 $4', $facility['phone']);
        $message .= xlt("Telefoon: ") . $phone . "<br />";
        $message .= text($facility['website']) . "<br /><br />";

        $message .= xlt("Bedankt dat we u van dienst mogen zijn") . "<br />";
        $message .= '<img src="cid:logo" alt="logo">';

        $mail = new MyMailer();

        $pt_email = $patientData['email'];
        $email_subject = xl('BeterKliniek Afspraak Bevestiging');
        ...
        if (file_exists("/var/www/localhost/htdocs/openemr/$ma_logo_path")) {
            $mail->AddEmbeddedImage('/var/www/localhost/htdocs/openemr/sites/default/images/logo.png', 'logo');
        }
        $fileonserver = file_exists($cat['email_url']);
        if ($cat['email_filename'] != null &&  file_exists($cat['email_url'])) {
            $mail->addAttachment($cat['email_url'], $cat['email_filename']);
        }
    ...
    function DOBandEncounter($pc_eid)
    {
        ...
        if ($patient_dob && $_POST['form_pid']) {
            sqlStatement(
                ...
                "pid = ?",
                array($patient_dob, $_POST['form_pid'])
            );
        }
    ...
    if ($eid) {
        $selfacil = '';
        $facility = sqlQuery(
            "SELECT pc_facility, pc_multiple, pc_aid, facility.name
    ...
    if ($_POST['form_action'] == "save") {
            ...
            if ($GLOBALS['select_multi_providers'] && $row['pc_multiple']) {
                ...
                if ($_POST['recurr_affect'] == 'current') {
                    ...
                    foreach ($providers_current as $provider) {
                        ...
                        $origEvent = sqlQuery(
                            "SELECT pc_recurrspec FROM openemr_postcalendar_events " .
                        ...
                        sqlStatement(
                            ...
                            " pc_recurrspec = ? " .
                            " WHERE pc_aid = ? AND pc_multiple=?",
                            array(serialize($oldRecurrspec), $provider, $row['pc_multiple'])
                        );
                    }
                    ...
                    foreach ($providers_current as $provider) {
                        ...
                        if (strcmp($_POST['event_start_date'], $_POST['selected_date']) != 0) {
                            ...
                            sqlStatement(
                                ...
                                " pc_enddate = ? " .
                                " WHERE pc_aid = ? AND pc_multiple=?",
                                array($selected_date, $provider, $row['pc_multiple'])
                            );
                ...
                if ($_POST['recurr_affect'] == 'current') {
                    ...
                    sqlStatement(
                        ...
                        " pc_recurrspec = ? " .
                        " WHERE pc_eid = ?",
                        array(serialize($oldRecurrspec), $eid)
                    );
         ...
         * ======================================================*/
            $eid = InsertEventFull();
            if ($GLOBALS['email_apt_info'] && $_POST['send_email']) {
                $appttime = $_POST['form_hour'] . ':' .  $_POST['form_minute'] ;
                emailLogin($_POST['form_pid'], $event_date, $appttime, $_POST['form_provider'], $_POST['form_category']);
            }
    ...
    if ($eid) {
        ...
        $row = sqlQuery(
            "SELECT e.*, u.fname, u.mname, u.lname " .
        ...
        if ($userid) {
            if ($_SESSION['pc_facility']) {
                $pref_facility = sqlFetchArray(
                    sqlStatement(
                        "SELECT f.id as facility_id,	f.name as facility FROM facility f WHERE f.id = ?",
                ...
                $pref_facility = sqlFetchArray(
                    sqlStatement(
                        "SELECT u.facility_id, f.name as facility FROM users u LEFT JOIN facility f on (u.facility_id = f.id) WHERE u.id = ?",
                        array($userid)
                    )
                );
            }
    ...
    if ($patientid) {
        ...
        if ($prow['email']) {
            $patienttitle['email'] = xl("Email") . ": " . $prow['Email'];
        }
    }
    ...
    $ures = sqlStatement(
        "SELECT id, username, fname, lname FROM users WHERE " .
        "authorized != 0 AND active = 1 ORDER BY lname, fname"
    );

    // Get event categories.
    $cres = sqlStatement(
        "SELECT pc_catid, pc_catname, pc_recurrtype, pc_duration, pc_end_all_day " .
    ...
    <script>
        ...
        $cres = sqlStatement(
            "SELECT pc_catid, pc_cattype, pc_catname, " .
